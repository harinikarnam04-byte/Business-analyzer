...
st.markdown("""
    ...
    .stButton>button { border-radius: 20px; font-weight: bold; background-color: #38bdf8 !important; color: #020617 !important; width: 100%; }
    /* Fix for overlapping table/chart on small screens */
...
with st.sidebar:
    ...
    st.markdown("---")
    idea = st.text_input("Venture Idea", placeholder="e.g. EV Charging Hub")
    ...
    if st.button("🚀 ANALYZE VENTURE"):
        ...
        with st.spinner("🤖 Simulating Business Model..."):
            prompt = f"""
            Role: MBA Executive Consultant. 
            Venture: {idea} in {location}. Target: {target}. Budget: ₹{budget}. 
            Industry: {industry}, Stage: {stage}. Language: {lang}. 
            
            Strictly ensure perfect spelling and professional formatting. 
            
            Deliver the report in 3 Parts:
            PART 1: Strategic Analysis (SWOT and 4Ps Marketing Mix for {target}).
            PART 2: Financial Growth (Funding roadmap, ROI Timeline, and Success Rate %). 
            Include projected Profits/Expenses for Year 2 and Year 3.
            PART 3: Risk & Pitch (Detailed Risk Recovery plan if loss occurs, and 30-sec Investor Pitch).
            """
...
# --- PAGE 2: FINANCIAL DASHBOARD (FIXED OVERLAP) ---
elif st.session_state.page == 2:
    st.markdown("### 💹 Step 2: Capital Allocation")
    
    allocs = [0.4, 0.25, 0.2, 0.15]
    side_df = pd.DataFrame({"Category": ["Setup", "Ops", "Marketing", "Cash"], "Amount": [budget*a for a in allocs]})
    
    # Chart with optimized spacing to prevent overlap
    fig = px.pie(side_df, values='Amount', names='Category', hole=0.5, 
                 color_discrete_sequence=px.colors.sequential.Tealgrn)
    fig.update_layout(
        showlegend=True, 
        paper_bgcolor='rgba(0,0,0,0)', 
        font=dict(color="white"),
        legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5), # Moves legend below to avoid overlap
        margin=dict(t=10, b=80, l=10, r=10) # Added bottom margin for legend
    )
...
# --- PAGE 3: VERDICT & RECOVERY ---
    ...
    st.markdown("---")
    st.download_button(label="📄 Download Full Report", data=st.session_state.report, file_name=f"BizVenture_Report.txt")
